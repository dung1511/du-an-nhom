...
from rooms.serializers import ReservationCheckInSerializer
...
@pytest.mark.django_db
class TestReservationCheckInAPIView:
    """Tests for check-in endpoint."""

    def test_checkin_blocks_unconfirmed_momo_qr_deposit(self):
        ...
        data = {
            ...
            'actual_check_in_date': date.today().isoformat(),
        }

        serializer = ReservationCheckInSerializer(data=data)

        assert serializer.is_valid() is False
        assert 'chưa chuyển cọc' in str(serializer.errors).lower()
